...
## Как выглядит работа бота

Бот последовательно задаёт пять вопросов о компании, текущем процессе, интеграциях, функциях AI и ожидаемом результате. Ниже показан фрагмент реального сценария с первыми тремя вопросами из `handlers/interview.py`; компания и ответы синтетические.

![Telegram-бриф AI Presale Assistant](docs/telegram-brief.png)

После завершения брифа AI формирует структуру решения, а отдельный pricing engine рассчитывает стоимость по проверяемым правилам. В демонстрационном примере для системы аналитики Wildberries и Ozon итоговая стоимость составляет **122 000 ₽**, срок — **15–25 рабочих дней**.

![Страница коммерческого предложения со стоимостью](docs/commercial-proposal-price.png)

[Открыть полный демонстрационный PDF](docs/commercial-proposal-demo.pdf)

Все материалы в этом разделе содержат только демонстрационные данные. Внутренние ключи, реальные заявки и клиентская переписка в репозиторий не включаются.
