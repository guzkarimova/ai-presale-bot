...
flowchart TD
    ...
    B --> C[FSM опрос: 5 вопросов]
    C --> D[Сохранение в SQLite]
    D --> E[Дублирование в Google Sheets]
    E --> F[AI-анализ заявки]
    F --> G[Выбор нужных сервисов/ID]
    G --> H[Pricing engine]
    H --> I[Генерация коммерческого предложения PDF]
    I --> J[Отправка менеджеру]
    J --> K[Логирование и fallback]

    F --> L{Проверка качества данных}
    L -->|Норма| G
    L -->|Ошибка| M[Ручная проверка / уведомление]
```
